...
fun validateThemeSemanticContrast(
    ...
    val pairs = listOf(
        ...
        SemanticContrastPair("Analytics Warning on Surface", "Analytics", "Warning", colors.analytics.warning, "Surface", colors.surfaces.surfaceRaised, 45.0, 3.0, false),
        SemanticContrastPair("Analytics Positive on Surface", "Analytics", "Positive", colors.analytics.positive, "Surface", colors.surfaces.surfaceRaised, 45.0, 3.0, false),
        SemanticContrastPair("Analytics Negative on Surface", "Analytics", "Negative", colors.analytics.negative, "Surface", colors.surfaces.surfaceRaised, 45.0, 3.0, false),

        // 7. Functional Borders & Dividers
        SemanticContrastPair("Focus Ring on Surface", "Interactions", "Focus Ring", colors.interaction.focus, "Surface", colors.surfaces.surface, 40.0, 2.5, false),
        SemanticContrastPair("Prominent Border on Surface", "Borders", "Prominent Border", colors.borders.prominent, "Surface", colors.surfaces.surface, 40.0, 2.5, false),
...
fun computeEnvironmentalSpecularEdges(
    ...
    val vCenter = ((screenOffsetY + componentHeight / 2f) / screenH).coerceIn(0f, 1f)
    ...
    val lumMR = sampleLuminanceField(uRight, vCenter, luminanceField, zonalLuminance, fallbackLuminance)
